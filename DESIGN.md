...
- **Sweep vs Base had no contain tuning at all.** `setSweepContain` never had a `case "base"` — it fell through to the pre-tuning default, wider and slower than even the intentionally light `pass_rush`/`soft_zone` boxes. One session: two sweeps broke for +51.8 and +64.5 vs base/cover3, one for +72.4 vs base/cover2. Folded into the light-box case now. Note: the scripted `Input{DX:0.4,DY:1}` balance probe never reproduced the break either before or after this fix — the exploit needs real player steering to show up, so the tightened tests here are a hygiene floor, not proof; watch the next live session.
- **Sweep contain was chasing a point fixed at the snap, not the ball.** The one place contain re-checked the ball's real position (`bp.X > forceX+1.4 → forceX = bp.X+1.1`) was gated to light fronts + Cover 3/Man Free only — Cover 2 never got it, `base` never got it. Technique: bounce it outside, even backward a step or two (contain doesn't retreat with you — `ty` is pinned near the LOS on purpose — so widening while he's frozen puts you past his fixed spot before he ever reacts). One session: `blitz+cover2` avg +47 (max 76.3) and `pass_rush+cover2` avg +36.8 (max 74.4) on a combined 12 attempts. Made the re-check universal instead of a special case. Scripted straight-line and fixed 3-phase bounce probes both failed to reproduce the break, before or after — same blind spot as the base fix. Needs a real game to confirm.
- **Remember, don’t patch unless they loop:** IZ crease vs Man Free / base; Post / PA Post vs Cover 2 if you hold it; cold Glance vs Cover 2.
